...
<script type="text/javascript">
		var url;
		function newUser(){
			$('#dlg').dialog('open').dialog('setTitle','New Organization');
			$('#fm').form('clear');
			url = 'save_user.php';
		}
		function editUser(){
			...
			if (row){
				$('#dlg').dialog('open').dialog('setTitle','Edit Organization');
				$('#fm').form('load',row);
				url = 'update_user.php?org_id='+row.org_id;
			}
		}
		function saveUser(){
			$('#fm').form('submit',{
		...
		function removeUser(){
			var row = $('#dg').datagrid('getSelected');
			if (row){
				$.messager.confirm('Confirm','Are you sure you want to remove this Organization?',function(r){
					if (r){
						$.post('remove_user.php',{org_id:row.org_id},function(result){
							if (result.success){
								$('#dg').datagrid('reload');	// reload the user data
							} else {
...
<div id="wrap"> 
  ...
  <header class="head">
    ...
    <div class="main-bar">
      <div class="container-fluid">
        <div class="row-fluid">
          <div class="span12">
            <h3><i class="icon-check-empty"></i>Organization</h3>
          </div>
        </div>
        <!-- /.row-fluid --> 
      </div>
  ...
  <div id="left"> 
    <!-- .user-media -->
    <div class="media user-media hidden-phone"> <a href="#" class="user-link"> <img src="../assets/img/user.gif" alt="User" class="media-object img-polaroid user-img"> <span class="label user-label">16</span> </a>
    ...
    <ul id="menu" class="unstyled accordion collapse in">
      ...
      <li class="accordion-group"> <a data-parent="#menu" data-toggle="collapse" class="accordion-toggle" data-target="#component-nav"> <i class="icon-tasks icon-large"></i>Asset Register <span
      ...
      <li class="accordion-group active "> <a data-parent="#menu" data-toggle="collapse" class="accordion-toggle" data-target="#error-nav"> <i class="icon-warning-sign icon-large"></i> Organisation <span
                                class="label label-inverse pull-right">2</span> </a>
        <ul class="collapse" id="error-nav">
          <li><a href="#"><i class="icon-angle-right"></i>Organisation Details</a></li>
      ...
      <div class="row-fluid"> 
        ...
        <div class="span12 inner"> 
          ...
          <div class="demo-info" style="margin-bottom:10px">
            ...
            <div>Identify the main business functions and decide what to include or 
              exclude.</div>
          </div>
          <table id="dg" title="Preparing Scope" class="easyui-datagrid" style="width:auto;height:250px"
            ...
            <thead>
              <tr>
                <th field="org_name" width="15%" scope="row">Organization Name</th>
                <th field="address" width="50%" scope="row">Address</th>
                <th field="telephone" width="10%" scope="row">Telephone</th>
                <th field="email" width="25%" scope="row">Email</th>
              </tr>
            </thead>
          </table>
          <div id="toolbar"> <a href="#" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="newUser()"> Add New</a> <a href="#" class="easyui-linkbutton" iconCls="icon-edit" plain="true" onclick="editUser()">Edit</a> <a href="#" class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="removeUser()">Remove</a> </div>
          <div id="dlg" class="easyui-dialog" style="width:400px;height:280px;padding:10px 20px"
           ...
           <div class="fitem">
                ...
                <input name="org_name" class="easyui-validatebox" >
              </div>
              <div class="fitem">
                <label>Address</label>
